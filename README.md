...
1. ***What is the advantage of the ChunkList approach as opposed to a standard 
  the link list implementation?*** (1 point)
   > Random access of an element in a chunk in a chunkList is possible, which makes it more efficient than accessing an element in a linkedlist because you need to traverse an 'n' number of nodes to reach the element in linked List.
   > You will have instant access to an element in a chunk
...
2. ***What would be the implications of increasing the size of ARRAY_SIZE to a 
   ...
   > The cost of traversing all elements of an array in a chunk in the chunkList is increased thus ultimately increasing the time complexity.
...
4. ***Compare placing a new element into the *FIRST* available empty space 
   ...
   > If an element is placed at the tail node, there is an efficiency overhead since you have to traverse all nodes in the list to reach the tail node.
